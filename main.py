...
def validate_settings(settings):
    ...
    if not settings.storage_channel_id:
        missing.append("STORAGE_CHANNEL_ID / STORAGE_CHAT_ID")
    if not settings.confirmation_chat_id:
        missing.append("CONFIRMATION_CHAT_ID / CONTROL_CHAT_ID / STORAGE_CHANNEL_ID")
...
def main():
    ...
    LOGGER.info(
        "Startup config loaded | intake=%s | storage=%s | confirmation=%s",
        settings.intake_channel_id,
        settings.storage_channel_id,
        settings.confirmation_chat_id,
    )
